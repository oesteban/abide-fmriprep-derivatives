...
using \emph{fMRIPrep} 25.2.4 (\citet{fmriprep1}; \citet{fmriprep2};
...
\emph{TemplateFlow} \citep[25.0.4,][]{templateflow}: \emph{ICBM 152
...
with six degrees of freedom. BOLD runs were slice-time corrected to the
mid-timepoint of each run's slice acquisition range using \texttt{3dTshift}
from AFNI \citep[RRID:SCR\_005927]{afni}. Several confounding
...
\emph{Grayordinate} ``dscalar'' files containing 91k samples were
resampled onto fsLR using the Connectome Workbench \citep{hcppipelines}.
\end{description}
